...
const TrendingCoins = async () => {
  ...
  try {
    ...
    console.error("Failed to fetch TrendingCoins:", error);
  ...
  const columns: DataTableColumn<TrendingCoin>[] = [
    ...
    },
    {
      ...
      cell: (coin) => {
        const item = coin.item;
        const isTrendingUp = item.data.price_change_percentage_24h.gbp > 0;
        ...
        return (
          <div className={cn("price-change", isTrendingUp ? "text-green-500" : "text-red-500")}>
            <p>
              ...
              {Math.abs(item.data.price_change_percentage_24h.gbp).toFixed(2)}%
            </p>
          </div>
        );
      },
